...
const submitAdd = async () => {
  ...
  try {
    await axios.post(`/api/tag/${props.tag.id}/create`, {
...
const discardTag = async () => {
  ...
  try {
    const res = await axios.get(`/api/tag/${props.tag.memberId}`);
    ...
    await axios.put(`/api/tag/${props.tag.id}/updateParent`, { newParentTagId: discarded.id });
    injOnRefresh();
  } catch (e) {
