...
export default function AddRecordModal({ tagTree, defaultTagId, onClose, onSaved }: AddRecordModalProps) {
  ...
  async function handleSave() {
    if (!tagId) { setError(t('addRecord.selectTagError')); return }
    setSaving(true)
    setError('')
    try {
    ...
    >
      <div style={{ width: '100%', maxWidth: 360, background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 12, padding: 24, display: 'flex', flexDirection: 'column', gap: 18, boxShadow: 'var(--shadow-modal)' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <span className="mono" style={{ fontSize: 10, color: 'var(--text-3)', letterSpacing: '0.15em', textTransform: 'uppercase' }}>{t('addRecord.title')}</span>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
          ...
          >
            <span style={{ flex: 1 }}>{selectedTag?.name ?? t('today.selectTag')}</span>
